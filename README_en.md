...
* **Tagging (labels)**: Add short colored tags to files directly from the sidebar.
    * Right-click a file and choose `Tag` to open the tag editor sheet.
    * The tag editor is WYSIWYG: As you type the tag name the preview's width adapts to the text; when the name is empty the preview is shown as a circular color swatch.
    * Default new tag color is red; you can choose from quick presets or fine-tune the RGBA sliders.
    * Tag text length is constrained for readability (Chinese: 5 chars, English: 10 chars).
    * Save to persist the tag; tags are shown inline in the sidebar next to the file name.
    * Double-click a tag on the list to open the tag dialog.
